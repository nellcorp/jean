...
- To replace a document's entire body, call update_outline_document with the full new markdown in `text` (editMode defaults to `replace`). Use editMode `append`/`prepend` only to add to the existing body. This is the sanctioned path — do not call the Outline REST API directly.
