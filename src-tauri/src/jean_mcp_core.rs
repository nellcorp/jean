...
pub fn tool_registry() -> Value {
    json!([
        ...
        {"name":"replace_outline_document_text","description":"Targeted find-and-replace inside an Outline document, so you can make a small edit without resending the whole document. Replaces the first occurrence of `find` with `replace` (set all=true for every occurrence). `find` must match exactly and be present, or the call errors. Use this for incremental edits; use update_outline_document only when replacing the entire body.","inputSchema":{"type":"object","properties":{"projectId":{"type":"string"},"documentId":{"type":"string"},"find":{"type":"string","description":"Exact substring to find in the document markdown."},"replace":{"type":"string","description":"Replacement text."},"all":{"type":"boolean","description":"Replace every occurrence (default false = first only)."}},"required":["projectId","documentId","find","replace"],"additionalProperties":false}},
